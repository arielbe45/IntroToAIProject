...
counter = 0
class QuoridorPlayerVsAIGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Quoridor Player vs AI Game")
        self.game = Game()
        ...
        # Start the AI turn if AI is the first player
        if self.game.current_player_turn == 1:  # Assuming player 1 is AI
    ...
    def move_player(self, x, y):
        ...
        if move.is_valid_move():
            ...
            self.root.after(1000, self.run_ai_turn)  # Run AI turn after player move
        else:
    ...
    def place_wall(self, x, y):
        ...
        if place_wall.place_wall(wall, self.game.white_player, self.game.black_player):
            ...
            self.draw_board()
            self.check_game_over()
            self.root.after(1000, self.run_ai_turn)  # Run AI turn after player places wall
        else:
...
if __name__ == "__main__":
    root = tk.Tk()
    app = QuoridorPlayerVsAIGUI(root)
    root.mainloop()
